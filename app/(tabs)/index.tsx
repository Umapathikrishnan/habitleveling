...
export default function Dashboard() {
    ...
    useEffect(() => {
        const checkStreak = async () => {
            ...
            try {
                ...
                if (lastSession && lastSession.ended_at) {
                    const lastDate = new Date(lastSession.ended_at);
                    ...
                    // If more than 1 day has passed (i.e., missed yesterday), reset streak
                    // diffDays = 0 (same day), 1 (yesterday), 2 (missed one day)
                    if (diffDays > 1 && profile.streak_current > 0) {
                        ...
                        // Update local state
                        setProfile((prev: any) => ({ ...prev, streak_current: 0 }));
                    }
                }
            } catch (err) {
        ...
        checkStreak();
    }, [profile?.id]); // Run when profile is loaded
